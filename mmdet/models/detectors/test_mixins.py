...
class BBoxTestMixin(object):
    ...
    def simple_test_bboxes_feature(self,
        ...
        if self.with_shared_head:
            roi_feats = self.shared_head(roi_feats)
        if with_decoder:
            cls_score, bbox_pred, _, _ , projected_features = self.bbox_head(roi_feats, bg_vector=bg_vector)
        else:
            cls_score, bbox_pred = self.bbox_head(roi_feats, bg_vector=bg_vector)
        img_shape = img_meta[0]['img_shape']
        scale_factor = img_meta[0]['scale_factor']
    ...
    def simple_test_bboxes_zsd(self,
                           x,
                           img_meta,
                           proposals,
                           rcnn_test_cfg,
        ...
        if self.with_shared_head:
            roi_feats = self.shared_head(roi_feats)
        seen_cls_score, seen_bbox_pred, unseen_cls_score, unseen_bbox_pred = self.bbox_head(roi_feats, bg_vector=bg_vector)

        img_shape = img_meta[0]['img_shape']
        scale_factor = img_meta[0]['scale_factor']
        det_bboxes, det_labels = self.bbox_head.get_det_bboxes_zsd(
            rois,
            seen_cls_score,
            seen_bbox_pred,
            unseen_cls_score,
